        ...
        <i-col span="12">
            <i-form :label-width="200" method="post">
                <i-form-item label="controller控制器">
                    <i-input name="controller_class_name" value="{{request()->controller_class_name}}" placeholder="ex: App\Http\Controllers\{module_name}\{controller_name}" :autocomplete="true"></i-input>
    ...
    {{--@if(isset($files))--}}
        ...
        @foreach ($files as $key=>$file)
            <i-modal v-model="modal{{$key}}" title="{{$file['path']}}" width="80">
                <pre><code class="php" v-html="modalcontent{{$key}}"></code></pre>
            </i-modal>
